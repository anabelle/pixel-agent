...
class NostrService {
  ...
  _isQualityAuthor(authorEvents) {
    ...
    if (authorEvents.length === 1) { const event = authorEvents[0]; return this._isQualityContent(event, 'general'); }
  ...
  _selectFollowCandidates(scoredEvents, currentContacts) {
    ...
    scoredEvents.forEach(({ evt, score }) => {
      ...
      if (evt.pubkey === this.pkHex) return;
    ...
    const qualityCandidates = candidates.filter(({ pubkey, score }) => {
      ...
      if (timeSinceReply < 2 * 60 * 60 * 1000) return false;
      return true;
    });
