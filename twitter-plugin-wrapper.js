 ...
 * but with enhanced rate limit handling.
 */
...
function parseRateLimitHeaders(headers) {
  ...
  if (isRateLimited) {
    ...
    console.warn(`[TWITTER WRAPPER] Rate limited detected. Pausing operations until ${rateLimitStatus.pausedUntil.toISOString()}`);
    console.warn(`[TWITTER WRAPPER] Rate limit details:`, {
...
function shouldPauseOperations() {
  ...
  if (isPaused) {
    ...
    console.warn(`[TWITTER WRAPPER] Operations paused. ${remaining} seconds remaining.`);
  }
...
class RateLimitAwareTwitterAuth {
  ...
  async isLoggedIn() {
    ...
    if (shouldPauseOperations()) {
      console.warn('[TWITTER WRAPPER] Skipping authentication check due to rate limit');
      return true;
    }
    ...
    try {
      ...
      if (error.code === 429 || error.statusCode === 429) {
        ...
        console.warn('[TWITTER WRAPPER] Authentication rate limited, continuing in read-only mode');
  ...
  async me() {
    ...
    if (shouldPauseOperations()) {
      console.warn('[TWITTER WRAPPER] Skipping profile fetch due to rate limit');
    ...
    try {
      ...
      if (error.code === 429 || error.statusCode === 429) {
        parseRateLimitHeaders(error.headers || error.response?.headers);
        console.warn('[TWITTER WRAPPER] Profile fetch rate limited, using cached profile');
        return this.profile || undefined;
      }
...
// Create a wrapper that mimics the original plugin structure
const twitterPluginWrapper = {
  // Copy all exports from the original plugin
  ...await import('@elizaos/plugin-twitter'),

  // Override the TwitterAuth class
...
export default twitterPluginWrapper;